"""
Direct Block Simulation (DBS) Panel
====================================
...
QVBoxLayout, QHBoxLayout, QGroupBox, QFormLayout, QSpinBox, QDoubleSpinBox,
    QComboBox, QPushButton, QLabel, QMessageBox, QWidget, QSplitter, QScrollArea, QFrame, QLineEdit,
...
class DBSPanel(BaseAnalysisPanel):
    ...
    def __init__(self, parent=None):
        ...
        super().__init__(parent=parent, panel_id="dbs")
        self.setWindowTitle("Direct Block Simulation (DBS)")
        self.resize(900, 700)
        ...
        try:
            ...
            if self.registry:
                ...
                if dh_signal is not None:
                    ...
                    logger.debug("DBSPanel: Connected to drillholeDataLoaded signal")
                
                vario_signal = self.registry.variogramResultsLoaded
                if vario_signal is not None:
                    vario_signal.connect(self._on_vario_loaded)
                    logger.debug("DBSPanel: Connected to variogramResultsLoaded signal")
    ...
    def _apply_variogram_results(self, res):
        ...
        if not (hasattr(self, 'v_type') and hasattr(self, 'rmaj') and hasattr(self, 'sill')):
            return
    ...
    def _setup_ui(self):
        ...
        self._create_block_group(s_lay)
        ...
        self.res_lbl = QLabel("DBS works at block support.")
        ...
        self.run_btn = QPushButton("RUN DBS")
    ...
    def _create_block_group(self, layout):
        ...
        # Block size (SMU)
        size_label = QLabel("Block Size / SMU (m):")
        size_label.setStyleSheet("color: #aaa; font-size: 9pt;")
        l.addWidget(size_label)
        
        h = QHBoxLayout()
        self.bdx = QDoubleSpinBox()
        self.bdx.setRange(0.1, 1000)
        self.bdx.setValue(10)
        self.bdy = QDoubleSpinBox()
        self.bdy.setRange(0.1, 1000)
        self.bdy.setValue(10)
        self.bdz = QDoubleSpinBox()
        self.bdz.setRange(0.1, 1000)
        self.bdz.setValue(5)
        h.addWidget(QLabel("DX:"))
        h.addWidget(self.bdx)
        h.addWidget(QLabel("DY:"))
        h.addWidget(self.bdy)
        h.addWidget(QLabel("DZ:"))
        h.addWidget(self.bdz)
        l.addLayout(h)
        ...
        self.var_btn = QPushButton("Calc Variance Reduction")
        self.var_btn.clicked.connect(self._calc_var)
        l.addWidget(self.var_btn)
        self.var_res = QLabel("-")
        self.var_res.setStyleSheet("color: #4fc3f7;")
        l.addWidget(self.var_res)
        layout.addWidget(g)
    ...
    def _auto_detect_grid(self):
        ...
        dx, dy, dz = self.bdx.value(), self.bdy.value(), self.bdz.value()
    ...
    def _create_vario_group(self, layout):
        ...
        f.addRow("Model:", self.v_type)
        f.addRow("Range:", self.rmaj)
        f.addRow("Sill:", self.sill)
        l.addLayout(f)
        layout.addWidget(g)

    def _create_sim_group(self, layout):
        g = QGroupBox("4. Params")
    ...
    def _on_data_loaded(self, data):
        ...
        if self.drillhole_data is not None:
            ...
            if hasattr(self, 'prop') and self.prop is not None:
                ...
                logger.debug("DBS panel: UI not ready, data stored for later initialization")

    def _calc_var(self):
        # Mock calculation
        try:
            from ..geostats.direct_block_sim import _compute_block_variance
            block_var = _compute_block_variance(
                self.sill.value(),
                self.rmaj.value(),
                self.bdx.value(),
                self.bdy.value(),
                self.bdz.value(),
                self.v_type.currentText().lower()
            )
            point_var = self.sill.value()
            reduction = (1 - block_var / point_var) * 100 if point_var > 0 else 0
            self.var_res.setText(f"Block variance: {block_var:.4f}\nReduction: {reduction:.1f}%")
        except Exception as e:
            self.var_res.setText(f"Error: {e}")

    def gather_parameters(self) -> Dict[str, Any]:
        return {
            'data_df': self.drillhole_data,
            'property': self.prop.currentText(),
            'block_dims': (self.bdx.value(), self.bdy.value(), self.bdz.value()),
            'variogram_type': self.v_type.currentText().lower(),
            'range': self.rmaj.value(),
    ...
    def run_analysis(self):
        """
        Run DBS following the standardized workflow:
        1. Load drillholes
        2. Define grid extents + block size
        3. Generate empty property array
        4. Run DBS to populate it
        """
        ...
        self._log_event(f"  Block size: {params['block_dx']:.1f}×{params['block_dy']:.1f}×{params['block_dz']:.1f}", "info")
        ...
        # ========================================================================
        # STEP 4: RUN DBS TO POPULATE IT
        # ========================================================================
        self._log_event("Step 4: Running DBS to populate array...", "progress")
        ...
        self.show_progress("Running DBS...")

        # Run with progress callback
        try:
            self.controller.run_dbs(
                params=params,
                callback=self.on_results
            )
        except Exception as e:
            logger.error(f"Failed to dispatch DBS: {e}", exc_info=True)
            self._log_event(f"ERROR: {str(e)}", "error")
            self.hide_progress()

    def on_results(self, payload):
        self._update_progress(100, "DBS simulation complete!")
        self._log_event("✓ Simulation finished successfully", "success")
        self.res_lbl.setText("DBS Simulation Complete.")
    ...
    def _visualize_results(self):
        ...
        try:
            ...
            if grid is not None:
                property_name = viz.get('property', 'DBS_Mean')
